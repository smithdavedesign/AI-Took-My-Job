#!/usr/bin/env bash
# Nexus AGENT_EXECUTION_COMMAND wrapper — gstack /canary skill
set -euo pipefail
...
echo "[gstack-canary] Starting /canary for execution ${NEXUS_AGENT_EXECUTION_ID:-local}"
cd "$WORKTREE"
node -e "
  const fs = require('fs');
  try {
...
if [ "$_OPENCLAW_READY" = "true" ]; then
  echo "[gstack-canary] Routing to openclaw agent --local (session: nexus-${NEXUS_AGENT_TASK_ID:-local})"
  openclaw agent --local \
    --session-id "nexus-${NEXUS_AGENT_TASK_ID:-$(date +%s)}" \
    --message "/canary $(cat "$PROMPT_FILE")" 2>&1
else
...
if [ -f "$OUTPUT_FILE" ]; then
  node -e "
    ...
    try {
      ...
      if (!out.findings || out.findings.length === 0) out.findings = ['canary complete'];
    ...
    } catch (e) { console.warn('[gstack-canary] sanitise failed:', e.message); }
  " 2>/dev/null || true
else
  node -e "const fs=require('fs');fs.writeFileSync('$OUTPUT_FILE',JSON.stringify({contractVersion:'nexus-agent-output-v1',summary:'canary complete',findings:['canary complete — check agent logs for full output'],outcome:'no-changes',changedFiles:[]},null,2));" 2>/dev/null || true
fi
echo "[gstack-canary] /canary complete"

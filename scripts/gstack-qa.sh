#!/usr/bin/env bash
# Nexus AGENT_EXECUTION_COMMAND wrapper — gstack /qa skill
set -euo pipefail
...
echo "[gstack-qa] Starting /qa for execution ${NEXUS_AGENT_EXECUTION_ID:-local}"
cd "$WORKTREE"
node -e "
  const fs = require('fs');
  try {
...
echo "[gstack-qa] Running gstack /qa..."
...
if [ "$_OPENCLAW_READY" = "true" ]; then
  echo "[gstack-qa] Routing to openclaw agent --local (session: nexus-${NEXUS_AGENT_TASK_ID:-local})"
  openclaw agent --local \
    --session-id "nexus-${NEXUS_AGENT_TASK_ID:-$(date +%s)}" \
    --message "/qa $(cat "$PROMPT_FILE")" 2>&1
else
  ...
  $CLAUDE_CMD /qa --print --dangerously-skip-permissions "$(cat "$PROMPT_FILE")" 2>&1
fi
if [ -f "$OUTPUT_FILE" ]; then
  node -e "
    ...
    try {
      ...
      if (!out.findings || out.findings.length === 0) out.findings = ['qa complete'];
    ...
    } catch (e) { console.warn('[gstack-qa] sanitise failed:', e.message); }
  " 2>/dev/null || true
else
  node -e "const fs=require('fs');fs.writeFileSync('$OUTPUT_FILE',JSON.stringify({contractVersion:'nexus-agent-output-v1',summary:'qa complete',findings:['qa complete — check agent logs for full output'],outcome:'changes-made',changedFiles:[]},null,2));" 2>/dev/null || true
fi
echo "[gstack-qa] /qa complete"

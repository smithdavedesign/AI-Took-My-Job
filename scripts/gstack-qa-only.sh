...
Your job: bug hunt — find functional bugs and document repro steps. No fixes.
...
if [ "$_OPENCLAW_READY" = "true" ]; then
  echo "[gstack-qa-only] Routing to openclaw agent --local (session: nexus-${NEXUS_AGENT_TASK_ID:-local})"
  openclaw agent --local \
    --session-id "nexus-${NEXUS_AGENT_TASK_ID:-$(date +%s)}" \
    --message "/qa-only $(cat "$PROMPT_FILE")" 2>&1
else
...
echo "[gstack-qa-only] Reverting any unexpected source changes..."
git -C "$WORKTREE" checkout -- . 2>/dev/null || true
git -C "$WORKTREE" clean -fd --exclude=.nexus 2>/dev/null || true
echo "[gstack-qa-only] Worktree clean."

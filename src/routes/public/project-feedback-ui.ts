...
import { normalizeCustomerPortalEmail } from '../../support/public-customer-portal-access.js';
...
interface PublicProjectSummaryInput {
  id: string;
  workspaceId: string;
  projectKey: string;
  name: string;
...
interface CustomerPortalGrantSummaryInput {
  id: string;
...
export async function buildPublicDashboardSummary(input: {
  app: FastifyInstance;
  project: PublicProjectSummaryInput;
...
export async function buildCustomerPortalSummary(input: {
  app: FastifyInstance;
  project: PublicProjectSummaryInput;
  grant: CustomerPortalGrantSummaryInput;
}): Promise<PublicDashboardSummary> {
  const recentReports = await input.app.reports.listRecent(200);
  const workspaceTriagePolicy = await input.app.workspaceTriagePolicies.findByWorkspaceId(input.project.workspaceId);
...
export function buildHostedFeedbackDashboardPage(project: { projectKey: string; name: string }, input: { accessToken: string; initialSummary: PublicDashboardSummary; summaryPath: string }): string {
  return [
    '<!DOCTYPE html>',
    '<html lang="en">',
    '<head>',
...
export function buildHostedFeedbackWidgetPage(project: { id: string; projectKey: string; name: string }, options: { embedded?: boolean; accessToken: string }): string {
  ...
  return [
    ...
    '    .inline-link { display: inline-block; color: var(--accent-dark); font-size: 0.9rem; font-weight: 500; text-decoration: none; }',
    ...
    '        <article class="card">',
    '          <h2>Operator</h2>',
    '          <p class="helper">Review and triage incoming reports in the Nexus operator dashboard.</p>',
    '          <div style="display:grid;gap:8px;margin-top:10px;">',
    '            <a class="inline-link" href="/learn/review-queue" target="_blank" rel="noopener">Open review queue ↗</a>',
    '            <a class="inline-link" href="/operator/login" target="_blank" rel="noopener">Operator login ↗</a>',
    '          </div>',
    '        </article>',
    '      </aside>',
    '    </section>',
...
export function buildHostedFeedbackEmbedScript(project: { projectKey: string; name: string }, accessToken: string): string {

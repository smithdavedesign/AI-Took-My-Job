export function buildReviewQueuePage(): string {
  return [
    ...
    '      <div class="hero-grid">',
    '        <div>',
    '          <h1>Operate the queue before anything is promoted into GitHub.</h1>',
    '          <p>This console is the decision point between launch and promotion: operators page through hosted feedback, surface project hotspots, and approve or reject each draft before Nexus creates downstream GitHub work.</p>',
    '        </div>',
    '        <div class="summary-grid">',
    '          <article class="summary-card"><span class="summary-label">Operate Queue</span><span id="queuedCount" class="summary-value">0</span><span class="summary-note" id="queuedNote">Pending reports across the current filter.</span></article>',
    '          <article class="summary-card"><span class="summary-label">Promote Risk</span><span id="criticalCount" class="summary-value">0</span><span class="summary-note">Critical hosted feedback waiting on review.</span></article>',
    ...
    '          <article class="summary-card"><span class="summary-label">Window</span><span id="pageValue" class="summary-value">1</span><span class="summary-note" id="pageNote">Queue page and sort state.</span></article>',
    '        </div>',
    '      </div>',
    '    </section>',
    '    <section class="shell">',
    ...
    '            <article id="guardrailTarget" class="guardrail-card guardrail-neutral"><strong>No report selected</strong><span>Select a queued report before making a promotion decision.</span></article>',
    '            <article id="guardrailContext" class="guardrail-card guardrail-neutral"><strong>Context not loaded</strong><span>Load report context so the operator can see evidence before acting.</span></article>',
    '            <article id="guardrailRationale" class="guardrail-card guardrail-neutral"><strong>Decision rationale missing</strong><span>Add notes so approval or rejection has explicit operator reasoning.</span></article>',
    '            <article id="guardrailPromotion" class="guardrail-card guardrail-neutral"><strong>Promotion action blocked</strong><span>Approval stays blocked until target, context, and rationale are visible.</span></article>',
    '          </div>',
    '        </div>',
    '        <div class="split">',
    '          <div>',
    ...
    '      const repositoryTarget = els.repository.value.trim() || (state.selected && state.selected.repository) || "";',
    '      renderGuardrailCard(els.guardrailTarget, state.selected && repositoryTarget ? "pass" : state.selected ? "fail" : "neutral", state.selected && repositoryTarget ? "Promotion target resolved" : state.selected ? "Promotion target missing" : "No report selected", state.selected && repositoryTarget ? `This action will target ${repositoryTarget}.` : state.selected ? "Resolve the repository target before approving downstream GitHub work." : "Pick a report before you decide anything.");',
    '      renderGuardrailCard(els.guardrailContext, state.context ? "pass" : state.selected ? "fail" : "neutral", state.context ? "Evidence is visible" : state.selected ? "Evidence still loading" : "Context not loaded", state.context ? "Context and review history are visible for operator inspection." : state.selected ? "Wait for the context load before promoting or rejecting work." : "Select a report to inspect evidence.");',
    '      renderGuardrailCard(els.guardrailRationale, els.notes.value.trim() ? "pass" : state.selected ? "fail" : "neutral", els.notes.value.trim() ? "Decision rationale recorded" : state.selected ? "Decision rationale missing" : "No rationale needed yet", els.notes.value.trim() ? "Operator notes are present for the next action." : state.selected ? "Add explicit notes before approving or rejecting the selected report." : "Notes become required once a report is selected.");',
    '      renderGuardrailCard(els.guardrailPromotion, canApproveSelection() ? "pass" : state.selected ? "fail" : "neutral", canApproveSelection() ? "Promotion action is unblocked" : state.selected ? "Promotion action is still gated" : "Promotion action is idle", canApproveSelection() ? "Approval can proceed because target, evidence, and rationale are all visible." : state.selected ? "Approval stays blocked until repository target, context, and notes are all present." : "Choose a report before acting.");',

...
function buildReviewQueuePage(): string {
  return [
    ...
    '    const state = { items: [], selected: null, context: null, summary: null, filters: { page: 1, limit: 12, sort: "newest", projectId: "", search: "" } };',
    '    const els = {',
    ...
    '    }',
    '    function saveAuth() {',
    '      localStorage.setItem(storageKey, JSON.stringify({ baseUrl: els.baseUrl.value.trim(), token: els.token.value.trim(), projectId: els.projectId.value.trim(), search: els.search.value.trim(), sort: els.sort.value }));',
    '    }',
    ...
    '      els.pageNote.textContent = `${filters.sort} order`; ',
    ...
    '      const activeFilters = [filters.projectId ? `project ${filters.projectId}` : null, filters.search ? `search \"${filters.search}\"` : null].filter(Boolean).join(" | ") || "No filters applied.";',
    ...
    '        card.innerHTML = [`<h3>${escapeHtml(item.title || item.reportId)}</h3>`,`<div class="meta">`,`<span class="badge">${escapeHtml(item.project ? item.project.name : "Unscoped project")}</span>`,`<span class="badge">${escapeHtml(item.severity)}</span>`,`<span class="badge">impact ${item.impactScore ?? "n/a"}</span>`,`<span class="badge">${escapeHtml(relativeTime(item.createdAt))}</span>`,`</div>`,`<div class="helper">${escapeHtml(item.repository || "repository unresolved")} | ${escapeHtml(item.reporterIdentifier || "anonymous")}</div>`].join("");',
